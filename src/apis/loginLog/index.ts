...
export const getTotalVisitorsRequest = async (totalVisitors: number) => {
    const result = await axios
    .get(TOTAL_VISITORS_URL)
...
export const getVisitorsTodayRequest = async (visitorsToday: number) => {
    const result = await axios
    .get(VISITORS_TODAY_URL)
    .then(requestHandler<VisitorCountDto>)
    .catch(requestErrorHandler);
    return result;
};