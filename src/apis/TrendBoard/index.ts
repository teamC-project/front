...
// function: 트렌드 게시물 좋아요 증가 API 함수 
export const increaseViewCountRequest = async (trendBoardNumber: number | string, accessToken: string) => {
	const result = await axios.patch(PATCH_TREND_BOARD_LIKE_COUNT_URL(trendBoardNumber), {}, bearerAuthorization(accessToken))
			.then(requestHandler<ResponseDto>)
			.catch(requestErrorHandler);
	return result;
};